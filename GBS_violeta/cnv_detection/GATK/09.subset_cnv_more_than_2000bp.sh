...
#subset the CNV that are at least 2Kbps
for i in segment_F*_cnv.vcf; do
        a=${i%_cnv.vcf}
        b=${a#segment_}
        awk '$11 >= 2000' ${b}_cnv_intermediate_lenght.vcf > ${b}_2K_filtered_cnv.vcf
done
...
for i in segment_F*_cnv.vcf; do
	...
	awk '{print $1, $2, $5, $8}' OFS="\t" ${b}_2K_filtered_cnv.vcf > $output_folder/${b}_2K_filtered_cnv.bed
done
